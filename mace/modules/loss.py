...
def mean_squared_error_charges(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # charges: [n_atoms]
    configs_weight = torch.repeat_interleave(
        ref.weight, ref.ptr[1:] - ref.ptr[:-1]
    )  # [n_atoms]
...
class EnergyLoss(torch.nn.Module):
    ...
    def forward(self, ref: Batch, pred: TensorDict) -> torch.Tensor:
        return weighted_mean_squared_error_energy(ref, pred)
...
class WeightedEnergyForcesChargesLoss(torch.nn.Module):
    ...
    def forward(self, ref: Batch, pred: TensorDict) -> torch.Tensor:
        return (
            self.energy_weight * weighted_mean_squared_error_energy(ref, pred)
            + self.forces_weight * mean_squared_error_forces(ref, pred)
            + self.charges_weight * mean_squared_error_charges(ref, pred)
        )
